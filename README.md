[README_SwarmShield_FL.md](https://github.com/user-attachments/files/28632336/README_SwarmShield_FL.md)
...
> Repository: `Dimitris-Gerakas-CERTH/p2pfl`, branch `malicious-nodes`  
> Entry point: `p2pfl/examples/fl_Health_Demo/`
...
```
p2pfl/examples/fl_Health_Demo/
├── trust_fl_experiment.py      # Main experiment script (all logic here)
├── cleaned_data/               # Patient CSVs — 22 files
│   ├── patient_01_clean.csv
│   ├── patient_02_clean.csv
│   └── ...
└── Trust_FL_Experiment_Colab-JUNE.ipynb   # Interactive Colab notebook
```

The notebook embeds `trust_fl_experiment.py` as a base64 blob and writes it to disk at runtime. This makes the notebook fully self-contained for Colab execution.
...
Open `Trust_FL_Experiment_Colab-JUNE.ipynb` in Google Colab. Run cells in order:
...
All pre-installed on Google Colab. For local use: `pip install torch pandas numpy`.
